...
Page({
  ...
  async getInfo(){
    let info
    try {
     info = await lostAndFind.doc(this.data.id).get();
    } catch (error) {
        wx.showToast({
          title: '获取信息失败',
          icon:"error"
  ...
  async postForm() {
    if (!this.data.goodstype  || !this.data.describe || !this.data.contact || !this.data.src || !this.data.position) {
      wx.showToast({
        title: '请正确输入值',
        icon: "error"
      })
      ...
      if (this.data.type == 'add') {
        lostAndFind.add({
          data: new app.globalData.model.LostAndFind({
        ...
        .update({
          data:
            new app.globalData.model.LostAndFind({
              goodstype: this.data.goodstype,
              describe: this.data.describe,
              contact: this.data.contact,
  ...
  onShareAppMessage: function () {
    return{
      ...
      path:"/pages/publish/lostAndFind/lostAndFind?type=info&id"+this.data.id,
